...
import open_clip
from yolo_utils import get_tensor_anchors, build_target, eval_iou_acc, bbox_iou
from ground_siglip import Encoder as SigLIPModel
from ground_cvos import LPNGeoLite, DetGeoLite, SampleGeoLite, TROGeoLite
...
def evaluate_model(
    ...
    for query_path, search_path, name, heading in tqdm(
        ...
        gt_heading = get_gt_heading(query_path, name)
        heading_target = torch.tensor(
            [heading_to_target(gt_heading)], dtype=torch.float32, device=config.device
        )
        bbox_pixel = [
        ...
        if isinstance(output, torch.Tensor):
            ...
            pred_heading = output[2] if len(output) > 2 else None
    ...
    metrics = {
        ...
        "mean_heading_mse": float(heading_meter.avg)
        if heading_meter.count > 0
        else None,
    ...
    if metrics["mean_heading_mse"] is not None:
...
def compare_all_models(checkpoints: Dict[str, str], config: EvalConfig):
    ...
    print("=" * 80)
    has_heading = any(m.get("mean_heading_mse") is not None for m in all_metrics)
    if has_heading:
        print(
            f"{'Model':<15} {'Accu50':<10} {'Accu25':<10} {'Mean IoU':<10} {'Unified IoU':<14} {'Center Dist':<12} {'Heading MSE':<12} {'Samples':<10}"
        )
        print("-" * 115)
    else:
        print(
            f"{'Model':<15} {'Accu50':<10} {'Accu25':<10} {'Mean IoU':<10} {'Unified IoU':<14} {'Center Dist':<12} {'Samples':<10}"
        )
        print("-" * 95)

    for m in all_metrics:
        heading_str = (
            f"{m.get('mean_heading_mse', 0):<12.4f}"
            if m.get("mean_heading_mse") is not None
            else "N/A"
        )
        unified_iou_str = f"{m.get('mean_unified_iou', 0):<14.4f}"
        if has_heading:
            print(
                f"{m['model_type']:<15} {m['accu50']:<10.4f} {m['accu25']:<10.4f} "
                f"{m['mean_iou']:<10.4f} {unified_iou_str} {m['mean_center_dist']:<12.4f} {heading_str} {m['num_samples']:<10}"
            )
        else:
            print(
                f"{m['model_type']:<15} {m['accu50']:<10.4f} {m['accu25']:<10.4f} "
                f"{m['mean_iou']:<10.4f} {unified_iou_str} {m['mean_center_dist']:<12.4f} {m['num_samples']:<10}"
            )

    best_iou_model = max(all_metrics, key=lambda x: x["mean_iou"])
    best_unified_iou_model = max(
        all_metrics, key=lambda x: x.get("mean_unified_iou", 0)
    )
    print(
        f"\nBest Model (by IoU): {best_iou_model['model_type']} ({best_iou_model['mean_iou']:.4f})"
    )
    print(
        f"Best Model (by Unified IoU): {best_unified_iou_model['model_type']} ({best_unified_iou_model.get('mean_unified_iou', 0):.4f})"
    )
...
def main():
    ...
    parser.add_argument(
        "--checkpoint",
        type=str,
        default="/data/feihong/ckpt/ground_siglip/best.pth",
