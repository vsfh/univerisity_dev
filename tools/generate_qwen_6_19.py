...
DEFAULT_OUTPUT_NAME = "qwen_6_19_description.json"
DEFAULT_SUMMARY_NAME = "qwen_6_19_description_summary.json"
...
HEIGHT_PROMPT = (
    ...
    "clues for inferring one unified overhead layout. The top of the satellite "
    "map is north.\n\n"
    "Write for a SigLIP2 text encoder and satellite-map search. Output exactly "
    "5 English retrieval segments about the same unified place, not four "
    "separate views. The five segments should emphasize slightly different "
    "retrieval cues: segment 1 overall layout and dominant land-cover pattern; "
    "segment 2 building footprint, roof shape, roof color, orientation, and "
    "building grouping; segment 3 roads, intersections, parking areas, paved "
    "yards, and access geometry; segment 4 green space, tree belts, water, "
    "sports fields, plazas, open ground, and surrounding context; segment 5 "
    "spatial relationships between the target and nearby map-visible features, "
    "using words such as north, south, east, west, center, along, adjacent to, "
    "between, and surrounded by.\n\n"
    "Each segment must be made of three short clauses joined by one semicolon. "
    "Each whole segment should use about 64 SigLIP2-style text tokens, "
    "keep the three clauses balanced, roughly 20 to 21 tokens each. "
    "Make every clause compact, concrete, and useful for retrieval rather than "
    "a report, caption, checklist, or explanation.\n\n"
    "Use only stable, satellite-visible evidence shared or strongly supported "
    "across the four images: building footprint, roof color and shape, facade "
    "or pavement color when visible, road color and geometry, parking lots, "
    ...
    "concrete color and layout words when useful, such as gray, white, red, "
    "blue, green, tan, black, light, dark, brick, asphalt, concrete, grass, "
    "rectangular, L-shaped, north, south, east, west, center, near, along, "
    "adjacent to, between, or surrounded by.\n\n"
    ...
    "functions, or facts not visible from the images. Do not make temporary "
    "details the main signal: vehicles, pedestrians, construction material, "
    "and lighting changes should be ignored unless they are unusually prominent "
    "and consistent across all four views. If views disagree, silently discard "
    "conflicting, weak, or unstable cues; keep only shared and stable evidence "
    "in the retrieval text.\n\n"
...
def generate_descriptions_for_one_dir(
    ...
    description_segments: Dict[str, List[Dict[str, Any]]] = {}
    ...
    for start in range(0, len(height_items), generation_batch_size):
        ...
        for (height, _), output_text in zip(batch_items, batch_outputs):
            description_segments[height] = structure_description_segments(output_text)
...
def main() -> None:
    ...
    if has_single_input:
        ...
        if existing_result is not None:
            ...
            if "description_segments" in existing_result:
                print(
                    json.dumps(
                        existing_result["description_segments"],
                        indent=2,
                        ensure_ascii=False,
                    )
                )
    ...
    if has_single_input:
        ...
        print(json.dumps(result["description_segments"], indent=2, ensure_ascii=False))
        return
