...
from grounding.legacy.utils.utils import *
...
class YOLOLayer(nn.Module):
    ...
    def forward(self, x, targets=None):
        ...
        if targets is not None:
            ...
            if x.is_cuda:
                self.mse_loss = self.mse_loss.to(x.device)
                self.bce_loss = self.bce_loss.to(x.device)
                self.ce_loss = self.ce_loss.to(x.device)
...
class Darknet(nn.Module):
    ...
    def __init__(self, config_path='/media/data1/feihong/remote/ckpt/yolov3_rs.cfg', img_size=1024, obj_out=False):
...
if __name__ == "__main__":
    ...
    model = Darknet()
    model.load_weights('/media/data1/feihong/remote/ckpt/yolov3.weights')
    # model.eval()
