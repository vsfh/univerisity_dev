...
def main(save_path, label_smooth=True):
    ...
    img_to_text_dict = json.load(open('../ckpt/drone_text_single_long.json', 'r'))
    ...
    logit_scale = torch.tensor(1.0).to(DEVICE)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for i, batch in enumerate(progress_bar):
            ...
            # loss = img_loss + text_loss + text_img_loss
            ...
            writer.add_scalar('Loss/drone2satellite', img_loss.item(), epoch * len(train_dataloader) + i)
            writer.add_scalar('Loss/text2satellite', text_loss.item(), epoch * len(train_dataloader) + i)
            # writer.add_scalar('Loss/drone2text', text_img_loss.item(), epoch * len(train_dataloader) + i)
        avg_loss = total_loss / len(train_dataloader)
            
        model.eval()
        img_val_loss, txt_val_loss, text_img_val_loss, val_avg_loss = validation(model, test_dataloader, epoch, logit_scale)
        writer.add_scalar('Loss/val_img_epoch', img_val_loss, epoch)
        writer.add_scalar('Loss/val_txt_epoch', txt_val_loss, epoch)
        writer.add_scalar('Loss/val_text_img_epoch', text_img_val_loss, epoch)
...
def validation(model, loader, epoch, logit_scale):
    ...
    for batch in progress_bar:
        ...
        with torch.no_grad():
            ...
            img_loss = info_nce_loss(anchor_feats, candidate_feats, positive_indices)
            text_loss = info_nce_loss(text_feats, candidate_feats, positive_indices)
            text_img_loss = compute_siglip_loss(anchor_feats, text_feats, logit_scale)
            val_loss = info_nce_loss(anchor_feats+text_feats, candidate_feats, positive_indices)

            total_loss += val_loss.item()
            img_total += img_loss.item()
            text_total += text_loss.item()
            text_img_total += text_img_loss.item()
    img_avg_loss = img_total / len(loader)
    txt_avg_loss = text_total / len(loader)
    text_img_avg_loss = text_img_total / len(loader)

    val_avg_loss = total_loss / len(loader)

    return img_avg_loss, txt_avg_loss, text_img_avg_loss, val_avg_loss
...
def eval(run=False):
    ...
    if run:
        ...
        encoder.load_state_dict(torch.load("../ckpt/train_add_loss/best.pth", map_location='cpu'))
        encoder.eval()
        ...
        np.savez("../ckpt/eval_search_loss.npz", **res_search)
        np.savez("../ckpt/eval_query_loss.npz", **res_query)
        np.savez("../ckpt/eval_text_loss.npz", **res_text)
        print("Evaluation complete.")
    else:
        search_res = np.load("../ckpt/eval_search_loss.npz")
        query_res = np.load("../ckpt/eval_query_loss.npz")
        text_res = np.load("../ckpt/eval_text_loss.npz")
        distances = json.load(open('../ckpt/distances.json','r'))
        test_num  = 100
        ...
        for key in tqdm(test_list):
            ...
            for img_name in ex_img_list:
                res[key].append(calcu_cos(drone_feature+text_feature, search_res[img_name]))
