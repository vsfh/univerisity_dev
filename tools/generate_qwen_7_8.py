...
DEFAULT_OUTPUT_NAME = "qwen_7_8_description.json"
DEFAULT_SUMMARY_NAME = "qwen_7_8_description_summary.json"
...
HEIGHT_PROMPT = (
    ...
    You are captioning four drone-view (aerial/oblique) images that show the SAME building or building complex. Your goal is to produce ONE structured, highly discriminative caption that can identify this exact place among similar aerial images.

    Use a region-aware captioning process internally, inspired by structured geo-visual description methods: first identify the global scene, then identify the main visible regions, then describe each region’s attributes and spatial relations. Do NOT assume bounding boxes, coordinates, OCR, labels, or text-region alignment. Use only visual evidence visible in the images.

    Write ONE dense paragraph within 200 words. Do not use bullets, labels, JSON, or a list. The paragraph should naturally include:

    1. Global scene: the dominant building or complex type and the most distinctive architectural feature.
    2. Main regions: visible components such as roof sections, wings, towers, domes, courtyards, entrances, lawns, paths, roads, tree rows, parking areas, or nearby buildings.
    3. Visual attributes: color, material, roof form, façade style, shape, symmetry, height contrast, and countable architectural details.
    4. Spatial relations: relative positions such as center, upper-left, lower-right, left of, right of, behind, in front of, surrounding, enclosed by, flanked by, connected to, or adjacent to.
    5. Discriminative layout: the specific arrangement of structures and surrounding landscape that makes this place distinguishable from similar aerial building scenes.

    Rules:
    - Lead with the single most distinctive visual element.
    - Use concrete, countable details when visible.
    - Prioritize evidence shared across all four images.
    - Avoid generic filler such as “a large building” or “an aerial view of a campus.”
    - Do not guess institution names unless clearly labeled.
    - Do not describe the sky, image quality, watermark, or camera angle.
    - Output only the final caption, with no preamble.
    '''
    # '''
...
def structure_description_segments(output_text: str) -> List[Dict[str, Any]]:
    segments = [segment.strip() for segment in output_text.split("||")]
...
def structure_height_description(output_text: str) -> str:
    return " ".join(output_text.split()).strip()
...
def generate_descriptions_for_one_dir(
    ...
    height_descriptions: Dict[str, str] = {}
    ...
    for start in range(0, len(height_items), generation_batch_size):
        ...
        for (height, _), output_text in zip(batch_items, batch_outputs):
            height_descriptions[height] = structure_height_description(output_text)
    ...
    return {
        ...
        "height_descriptions": height_descriptions,
    }
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=(
            "Only process the first N sorted sample directories. "
            "Use <=0 to process all directories."
        ),
    )
    ...
    if has_single_input:
        ...
        if existing_result is not None:
            ...
            if "height_descriptions" in existing_result:
                print(
                    json.dumps(
                        existing_result["height_descriptions"],
                        indent=2,
                        ensure_ascii=False,
                    )
                )
        ...
        if args.max_cases > 0:
            all_sample_dirs = all_sample_dirs[: args.max_cases]
        shard_sample_dirs = [
            sample_dir
    ...
    if has_single_input:
        ...
        print(json.dumps(result["height_descriptions"], indent=2, ensure_ascii=False))
        return
