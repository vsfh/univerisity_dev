...
class ShiftedSatelliteDroneDataset(Dataset):
	...
	def __getitem__(self, idx: int) -> Dict[str, Any]:
		...
		center_x = (resized_bbox[0] + resized_bbox[2]) * 0.5 / float(target_w)
		center_y = (resized_bbox[1] + resized_bbox[3]) * 0.5 / float(target_h)
		col_idx = min(int(center_x * GRID_COLS), GRID_COLS - 1)
		row_idx = min(int(center_y * GRID_ROWS), GRID_ROWS - 1)
		index = row_idx * GRID_COLS + col_idx
