...
class Encoder(nn.Module):
    def __init__(self, model_name, proj_dim=768):
        ...
        # Load EVA-CLIP model
        try:
            self.model, _, _ = open_clip.create_model_and_transforms(model_name, cache_dir=CACHE_DIR)
...
def main(save_path):
    ...
    model, _, preprocess = open_clip.create_model_and_transforms(MODEL_NAME, cache_dir=CACHE_DIR)
...
def eval(run=False):
    ...
    if run:
        ...
        encoder = Encoder(model_name=MODEL_NAME, proj_dim=PROJECTION_DIM).to(DEVICE)
        model_path = f"../ckpt/train_evaclip/best.pth"
