...
Every invocation replaces **only outputs/heatmap_lambda_box_0p5/** and
trains all 15 experiments from scratch. Stop any previous runner before restarting.
Other output directories are untouched.

- Fixed lambda_box: 0.5.
- Outer lambda_heatmap: 0, 0.05, 0.1, 0.2, 0.4.
- Seeds: 42, 43, 44; 20 epochs per experiment from the existing baseline.
...
are all under outputs/heatmap_lambda_box_0p5/. Ephemeral DataLoader IPC uses /tmp
