...
class Config:
    ...
    TEXT_POOLER_ALIGN_WEIGHT = 0.046
    TEXT_POOLER_ALIGN_STOP_EPOCH = 10
    TEXT_POOLER_ALIGN_WARMUP_RATIO = 0.1
...
def text_pooler_align_weight(epoch: int) -> float:
    ...
    if stop_epoch <= 0 or epoch_number > stop_epoch:
        return 0.0
    return weight
...
def text_pooler_align_target() -> str:
    ...
    if target not in {"drone", "satellite","inner"}:
        raise ValueError(
            f"Invalid TEXT_POOLER_ALIGN_TARGET={Config.TEXT_POOLER_ALIGN_TARGET}. "
            "Choose 'drone' or 'satellite' or 'inner'."
        )
    return target
...
def add_heatmap_to_confidence(
    ...
    if (
        ...
        or Config.ENCODER_TYPE not in ENCODER_TYPES
...
def _colorize_heatmap_for_writer(heatmap: torch.Tensor) -> torch.Tensor:
    ...
    position = heatmap[:, 0] * (palette.shape[0] - 1)
    lower = position.floor().long()
    upper = (lower + 1).clamp_max(palette.shape[0] - 1)
    weight = (position - lower).unsqueeze(-1)
    color = palette[lower] * (1.0 - weight) + palette[upper] * weight
...
def build_encoder(use_ap: bool, usesg: bool = True) -> nn.Module:
    ...
    if Config.ENCODER_TYPE == "model_bi":
        return model_bi(**kwargs)
    if Config.ENCODER_TYPE == "ada":
        return Encoder_ada(**kwargs)
    return Encoder_test(**kwargs)
...
def within_image_text_alignment_loss(
    ...
    grid_feats = F.normalize(grid_feats, p=2, dim=2)
    logits = torch.einsum("bd,bnd->bn", text_feats, grid_feats) / temperature
    return F.cross_entropy(logits, local_indices)
...
def validate(
    ...
    for batch in tqdm(loader, desc="Validating", disable=not accelerator.is_main_process):
        ...
        positive_indices = torch.zeros(B, B * NUM_LOCATIONS, device=accelerator.device)
        
        batch_offsets = torch.arange(B, device=accelerator.device) * NUM_LOCATIONS
        row_indices_broad = torch.arange(B, device=accelerator.device).unsqueeze(1)
        col_offsets = torch.arange(NUM_LOCATIONS, device=accelerator.device).unsqueeze(0)
    ...
    for epoch in range(Config.NUM_EPOCHS):
        ...
        for i, batch in enumerate(progress_bar):
            ...
            with accelerator.accumulate(model):
                ...
                satellite_ids = batch["satellite_id"].to(
                    accelerator.device,
                    non_blocking=True,
                )
                ...
                with accelerator.autocast():
                    ...
                    if (
                        ...
                        and Config.ENCODER_TYPE in ENCODER_TYPES
                    ...
                    positive_indices = torch.zeros(
                        B, B * NUM_LOCATIONS, device=accelerator.device
                    )

                    batch_offsets = (
                        torch.arange(B, device=accelerator.device) * NUM_LOCATIONS
                    )
                    row_indices_broad = torch.arange(
                        B, device=accelerator.device
                    ).unsqueeze(1)
                    col_offsets = torch.arange(
                        NUM_LOCATIONS, device=accelerator.device
                    ).unsqueeze(0)
                    ...
                    image_retrieval_loss = info_nce_loss(
                        ...
                        candidate_feats,
                        positive_indices,
                        candidate_mask=image_retrieval_candidate_mask,
                    )
                    if (
                        Config.ENCODER_TYPE in ENCODER_TYPES
